...
.directive('friendSide', function() {
  return {
    restrict: 'E',
    template: '<div><h4>Friends List</h4><ul style="list-style: none; padding: 0;"><li ng-repeat="user in friendZone.users track by user.name" ng-click="friendZone.chooseFriend(user.name)" style="padding: 10px; margin: 5px; background: #f0f0f0; cursor: pointer;" ng-class="{selected: friendZone.isSelected(user)}">{{user.name}}</li></ul><div ng-if="friendZone.chosenFriend"><img ng-src="{{friendZone.chosenFriend.fbImgUrls[0]}}" style="width: 80px; height: 80px; border-radius: 8px;"></div></div>',
    controller: 'FriendController',
    controllerAs: 'friendZone'
  };
})
...
fbImgUrls:[
  ...
  friendZone.getMatchedFriends = function() {
    var filteredFriends = friendZone.users.filter(function(user) {
      return friendZone.chosenFriend && friendZone.chosenFriend.matches.indexOf(user.name) !== -1;
    });
